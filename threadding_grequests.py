...
def create_files(FPS, read_quaddump):
	for i in range(1,int(FPS)+1,1):
		file_format = ("quadDUMP_worker_{}.py".format(i))
...
def create_bash_executable(FPS, Name,password):
	open("helloword.sh", "w").close()
	print (open("helloword.sh", "r").read())
	bash = open("helloword.sh", "a")
	for i in range(1,int(FPS)+1,1):
		time_delay = i/10.0
		print (time_delay)
		name_executable = ("python quadDUMP_worker_{}.py {} {} {} & \n".format(i, Name,password, time_delay))
